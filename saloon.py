...
if 'Date' in repeat_customers.columns:
    ...
    fig = px.line(monthly_rev, x='Month', y='Net Revenue - Tax', markers=True, title="Revenue Over Time")
...
if 'Date' in repeat_customers.columns:
    ...
    fig2 = px.bar(visits_over_time, x='Month', y='Unique Repeat Customers', color='Unique Repeat Customers')
...
if 'Date' in daily_sms.columns:
    ...
    fig3 = px.area(sms_monthly, x='Month', y='SMS Count', title="Monthly SMS Activity")
...
st.markdown("---")
st.caption("Built for Naturals Bhimavaram • Real-Time Business Dashboard")
